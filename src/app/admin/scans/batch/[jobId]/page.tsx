...
export default async function AdminBatchJobPage({ params }: { params: Promise<{ jobId: string }> }) {
  await requireAdmin();
  ...
  return (
    <div className="space-y-8">
      <div>
        <Link href="/admin/scans" className="text-xs text-sky-400 hover:underline">
          ← Scan & score
        </Link>
        ...
        <p className="mt-2 text-sm text-slate-400">
          ...
          {job.uploadedByRole === "ADMIN" ? "Admin upload" : "School upload"}
          {job.school ? ` · ${job.school.name}` : ""}
        </p>
      </div>
          ...
          <tbody>
            {job.pages.map((p) => (
              <tr key={p.id} className="border-t border-white/5">
                ...
                <td className="px-4 py-3 font-mono text-xs text-sky-300">{p.examCode ?? "—"}</td>
